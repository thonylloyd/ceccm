    ...
    <section className="bg-light py-20 lg:py-24">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="text-center mb-14">
          ...
          {statement && (
            <p className="mt-6 max-w-3xl mx-auto text-charcoal/75 italic leading-relaxed">
              "{statement}"
            </p>
          )}
        </div>
        ...
        {cards.length > 0 && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {cards.map((c) => (
              <article
                key={c.id}
                className="bg-white border border-gold/60 px-6 py-9 text-center hover:shadow-card transition-shadow"
              >
